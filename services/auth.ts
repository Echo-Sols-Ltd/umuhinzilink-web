import { ApiResponse, User, LoginRequest, AuthResponse, UserRequest, FarmerRequest, SupplierRequest, BuyerRequest, Buyer, Supplier, Farmer } from '@/types'
...
export class AuthService {
    ...
    async registerFarmer(userData: FarmerRequest): Promise<ApiResponse<Farmer>> {
        const response = await apiClient.post<Farmer>(
            API_ENDPOINTS.AUTH.REGISTER_FARMER,
            userData
        )
        return response
    }
    ...
    async registerSupplier(userData: SupplierRequest): Promise<ApiResponse<Supplier>> {
        const response = await apiClient.post<Supplier>(
            API_ENDPOINTS.AUTH.REGISTER_SUPPLIER,
            userData
        )
        return response
    }
    ...
    async registerBuyer(userData: BuyerRequest): Promise<ApiResponse<Buyer>> {
        const response = await apiClient.post<Buyer>(
            API_ENDPOINTS.AUTH.REGISTER_BUYER,
            userData
        )
        return response
    }
