import { FarmerProduct, SupplierProduct, FarmerProductRequest, ApiResponse, PaginatedResponse, FarmerProductionStat, SupplierProductionStat, SupplierProductRequest } from '@/types';
...
class ProductService {
    ...
    async createSupplierProduct(payload: SupplierProductRequest): Promise<ApiResponse<SupplierProduct>> {
        return await apiClient.post<SupplierProduct>(API_ENDPOINTS.PRODUCT.CREATE_SUPPLIER, payload);
    }

    async updateFarmerProduct(id: string, payload: FarmerProductRequest): Promise<ApiResponse<FarmerProduct>> {
        return await apiClient.put<FarmerProduct>(API_ENDPOINTS.PRODUCT.UPDATE_FARMER(id), payload);
    }

    async updateSupplierProduct(id: string, payload: SupplierProductRequest): Promise<ApiResponse<SupplierProduct>> {
    ...
    async getFarmerBuyerProducts(): Promise<ApiResponse<PaginatedResponse<SupplierProduct[]>>> {
        return await apiClient.get<PaginatedResponse<SupplierProduct[]>>(API_ENDPOINTS.PRODUCT.FARMER_BUYER_ALL);
    }
